...
import pic1 from "@/image/Food/asian1.png";
import pic2 from "@/image/Food/asian2.png";
import pic3 from "@/image/Food/asian3.png";

export default function GalleryPage() {
  const images: StaticImageData[] = [pic1, pic2, pic3];
  ...
  return (
    <div className="min-h-screen bg-bgLogin relative">
      <header className="w-full bg-bgLogin text-white flex items-center gap-3 p-4 fixed top-0 left-0 z-20">
        ...
        <Link
          href="https://www.facebook.com/asiancowph/"
          target="_blank"
          rel="noopener noreferrer"
        >
          <span className="text-sm text-black sm:text-sm pl-3 cursor-default">
            Photo Courtesy: <strong className="hover:text-red-500">Asian Cow by Chef Patrick Co (2025)</strong>
          </span>
        </Link>
      ...
      <div className="min-h-screen flex justify-center items-center px-6 pb-20">
        <div
          className="grid gap-10 mt-20 place-items-center grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 max-w-[1500px]"
        >
          {images.map((img, index) => (
            <div
              key={index}
              onClick={() => setSelectedImage(img)}
              className="cursor-pointer rounded-xl overflow-hidden shadow-md hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group w-[260px] sm:w-[280px] md:w-[300px] lg:w-[320px]"
            >
